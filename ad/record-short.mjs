import { chromium } from 'playwright';
import fs from 'fs';

const BASE = 'http://localhost:4173';
const OUT = './video-short';
const W = 1920, H = 1080;

fs.rmSync(OUT, { recursive: true, force: true });

const t0 = Date.now();
const marks = [];
const beat = (label) => {
  const t = (Date.now() - t0) / 1000;
  marks.push({ t: +t.toFixed(2), label });
  console.log(`[${t.toFixed(2).padStart(6)}s] ${label}`);
};

const CURSOR_JS = `
const boot = () => {
  if (window.__adCursor) return;
  window.__adCursor = true;
  const style = document.createElement('style');
  style.textContent = \`
    #ad-cursor{position:fixed;left:0;top:0;width:30px;height:30px;z-index:2147483647;
      pointer-events:none;transform:translate(-2px,-2px);opacity:0;transition:opacity .25s}
    #ad-cursor.on{opacity:1}
    .ad-ripple{position:fixed;z-index:2147483646;pointer-events:none;border-radius:50%;
      width:14px;height:14px;margin:-7px 0 0 -7px;border:2px solid #0d7a72;
      animation:adr .55s ease-out forwards}
    @keyframes adr{from{transform:scale(.5);opacity:.9}to{transform:scale(3.4);opacity:0}}
    #ad-card{position:fixed;inset:0;z-index:2147483645;display:flex;flex-direction:column;
      align-items:center;justify-content:center;gap:18px;text-align:center;
      background:linear-gradient(150deg,#0b3f3b 0%,#0d7a72 55%,#0f8f85 100%);color:#fff;
      font-family:ui-serif,Georgia,serif;pointer-events:none;
      opacity:0;visibility:hidden;transition:opacity .45s,visibility 0s .45s}
    #ad-card.on{opacity:1;visibility:visible;transition:opacity .45s}
    #ad-card .adk{font:600 18px/1 ui-sans-serif,system-ui;letter-spacing:.22em;
      text-transform:uppercase;color:rgba(255,255,255,.82)}
    #ad-card h1{font-size:82px;line-height:1.08;margin:0;max-width:1340px;font-weight:700;color:#fff}
    #ad-card p{font:400 31px/1.5 ui-sans-serif,system-ui;margin:0;max-width:1000px;
      color:rgba(255,255,255,.92)}
    #ad-card .url{font:600 40px/1 ui-sans-serif,system-ui;letter-spacing:.02em;color:#fff;
      background:rgba(255,255,255,.16);padding:20px 40px;border-radius:999px;margin-top:8px}
    #ad-lower{position:fixed;left:64px;bottom:64px;z-index:2147483644;
      background:rgba(11,63,59,.94);color:#fff;padding:19px 30px;border-radius:18px;
      font:600 29px/1.3 ui-sans-serif,system-ui;letter-spacing:.01em;pointer-events:none;
      box-shadow:0 12px 34px rgba(0,0,0,.3);opacity:0;transform:translateY(10px);
      transition:opacity .3s,transform .3s;max-width:820px}
    #ad-lower.on{opacity:1;transform:translateY(0)}
  \`;
  document.head.appendChild(style);
  const c = document.createElement('div');
  c.id = 'ad-cursor';
  c.innerHTML = '<svg viewBox="0 0 22 22" width="30" height="30">' +
    '<path d="M2 1 L2 17 L6.2 13.1 L9.1 19.6 L12 18.2 L9.2 11.9 L15 11.7 Z" ' +
    'fill="#111" stroke="#fff" stroke-width="1.3" stroke-linejoin="round"/></svg>';
  document.body.appendChild(c);
  addEventListener('mousemove', e => {
    c.classList.add('on');
    c.style.left = e.clientX + 'px'; c.style.top = e.clientY + 'px';
  }, true);
  addEventListener('mousedown', e => {
    const r = document.createElement('div');
    r.className = 'ad-ripple';
    r.style.left = e.clientX + 'px'; r.style.top = e.clientY + 'px';
    document.body.appendChild(r);
    setTimeout(() => r.remove(), 600);
  }, true);
};
if (document.readyState === 'loading') addEventListener('DOMContentLoaded', boot);
else boot();
`;

const run = async () => {
  const browser = await chromium.launch();
  const ctx = await browser.newContext({
    viewport: { width: W, height: H },
    deviceScaleFactor: 1,
    recordVideo: { dir: OUT, size: { width: W, height: H } },
  });
  await ctx.addInitScript(CURSOR_JS);
  const page = await ctx.newPage();

  const wait = (ms) => page.waitForTimeout(ms);
  const moveTo = (x, y, steps = 22) => page.mouse.move(x, y, { steps });

  const centerOf = async (sel, idx = 0) => {
    const el = page.locator(sel).nth(idx);
    await el.scrollIntoViewIfNeeded();
    await wait(350);
    const b = await el.boundingBox();
    if (!b) throw new Error('no box: ' + sel);
    return [b.x + b.width / 2, b.y + b.height / 2];
  };

  const clickSel = async (sel, idx = 0, pause = 600) => {
    const [x, y] = await centerOf(sel, idx);
    await moveTo(x, y);
    await wait(200);
    await page.mouse.click(x, y);
    await wait(pause);
  };

  const clickText = async (sel, text, pause = 600) => {
    const el = page.locator(sel, { hasText: text }).first();
    await el.scrollIntoViewIfNeeded();
    await wait(300);
    const b = await el.boundingBox();
    await moveTo(b.x + b.width / 2, b.y + b.height / 2);
    await wait(200);
    await page.mouse.click(b.x + b.width / 2, b.y + b.height / 2);
    await wait(pause);
  };

  const lower = async (text, ms = 0) => {
    await page.evaluate((t) => {
      let el = document.getElementById('ad-lower');
      if (!el) { el = document.createElement('div'); el.id = 'ad-lower'; document.body.appendChild(el); }
      if (t === null) { el.classList.remove('on'); return; }
      el.textContent = t; el.classList.add('on');
    }, text);
    if (ms) await wait(ms);
  };
  const lowerOff = () => lower(null);

  const card = async (eyebrow, title, sub, url, holdMs) => {
    await page.evaluate(([e, t, s, u]) => {
      let el = document.getElementById('ad-card');
      if (!el) { el = document.createElement('div'); el.id = 'ad-card'; document.body.appendChild(el); }
      el.innerHTML = (e ? `<div class="adk">${e}</div>` : '') +
        (t ? `<h1>${t}</h1>` : '') + (s ? `<p>${s}</p>` : '') +
        (u ? `<div class="url">${u}</div>` : '');
      const cur = document.getElementById('ad-cursor');
      if (cur) cur.style.display = 'none';
      requestAnimationFrame(() => el.classList.add('on'));
    }, [eyebrow, title, sub, url]);
    await wait(holdMs);
  };

  const setNumber = async (sel, value) => {
    const [x, y] = await centerOf(sel);
    await moveTo(x, y);
    await wait(150);
    await page.mouse.click(x, y, { clickCount: 3 });
    await wait(150);
    await page.keyboard.type(String(value), { delay: 110 });
    await page.keyboard.press('Enter');
    await page.evaluate(() => getSelection().removeAllRanges());
    await wait(500);
  };

  // =========================================================================
  // SCENE 1 — Cold open on the live hero: poke it, swap the algorithm
  // =========================================================================
  await page.goto(`${BASE}/index.html`, { waitUntil: 'networkidle' });
  await wait(1200);
  beat('S1 hero');
  await lower('The same 21 points, four different algorithms', 0);

  // add two points inside the hero canvas and let each one re-solve
  const hero = await page.locator('.hero-live svg').first().boundingBox();
  const spots = [
    [hero.x + hero.width * 0.30, hero.y + hero.height * 0.72],
    [hero.x + hero.width * 0.68, hero.y + hero.height * 0.34],
  ];
  for (const [x, y] of spots) {
    await moveTo(x, y, 24);
    await wait(280);
    await page.mouse.click(x, y);
    await wait(1000);
  }
  beat('S1 swap algorithm');
  await lower('Click to add your own — every method disagrees', 0);
  await clickText('.hero-chip', 'DBSCAN', 1550);
  await clickText('.hero-chip', 'k-NN', 1550);
  await lowerOff();
  await wait(200);

  // =========================================================================
  // SCENE 2 — K-Means: run it, read your own numbers in the formula
  // =========================================================================
  beat('S2 k-means');
  await page.goto(`${BASE}/pages/algorithm.html?id=k-means`, { waitUntil: 'networkidle' });
  await wait(900);
  await lower('Every concept is a lab, not a diagram', 0);
  await page.evaluate(() => {
    const el = document.getElementById('partition-step');
    if (el) window.scrollTo({ top: el.getBoundingClientRect().top + scrollY - 230, behavior: 'smooth' });
  });
  await wait(1100);
  await page.locator('.preset-select').first().selectOption({ label: 'Two moons' });
  await wait(1300);
  await lowerOff();

  beat('S2 converge');
  await lower('Step it, or run it to convergence', 0);
  await clickSel('#partition-step', 0, 850);
  await clickSel('#partition-run', 0, 1700);
  await lowerOff();

  beat('S2 substituted math');
  await lower('The formula shows YOUR numbers', 0);
  await page.evaluate(() => window.scrollBy({ top: 430, behavior: 'smooth' }));
  await wait(2100);
  await lowerOff();

  beat('S2 change k');
  await lower('Change k — the whole page recomputes', 0);
  await page.evaluate(() => {
    const el = document.getElementById('partition-k');
    if (el) window.scrollTo({ top: el.getBoundingClientRect().top + scrollY - 240, behavior: 'smooth' });
  });
  await wait(900);
  await setNumber('#partition-k', 4);
  await clickSel('#partition-run', 0, 1500);
  await lowerOff();
  await wait(200);

  // =========================================================================
  // SCENE 3 — Backprop: train it, then flip to dark mode
  // =========================================================================
  beat('S3 backprop');
  await page.goto(`${BASE}/pages/algorithm.html?id=backpropagation`, { waitUntil: 'networkidle' });
  await wait(900);
  await lower('Build a network. Watch it learn.', 0);
  await page.evaluate(() => {
    const el = document.getElementById('bp-next');
    if (el) window.scrollTo({ top: el.getBoundingClientRect().top + scrollY - 235, behavior: 'smooth' });
  });
  await wait(1100);
  await clickSel('#bp-next', 0, 900);
  await clickSel('#bp-train', 0, 1900);
  await lowerOff();

  beat('S3 dark mode');
  await lower('Dark mode, because of course', 0);
  await clickSel('#theme-toggle', 0, 2100);
  await lowerOff();
  await wait(400);

  // =========================================================================
  // SCENE 4 — End card
  // =========================================================================
  beat('S4 end card');
  await card(
    '42 concepts · no signup',
    'Machine learning<br/>you can poke.',
    null,
    'learnvisually.fun',
    3400
  );
  beat('END');

  await ctx.close();
  await browser.close();

  const files = fs.readdirSync(OUT).filter(f => f.endsWith('.webm'));
  fs.writeFileSync('./timeline-short.json', JSON.stringify({ marks, video: files[0] }, null, 2));
  console.log('\nvideo:', files);
};

run().catch(e => { console.error(e); process.exit(1); });
